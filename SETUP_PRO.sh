...
echo -e "  └────────────────────────────────────────────────────────┘"
echo -e "\n  💡 尊敬的Aura Grid Pro用户："
echo -e "     您即将部署的是 Aura Grid 家族的最高商业级别平台。"
echo -e "     向导程序将自动梳理您的宿主环境并为您安全交接服务。"
echo -e "\n     🚀 ${BLUE}[温馨指引]${NC} 如果您在后续过程中："
echo -e "     1. 遇到网络拥堵、端口冲突等环境报错 —— 强烈建议复制日志，借助豆包/Deepseek/Claude/Gemini/ChatGPT 等大模型AI助手排查。"
...
if [ "$USE_ALIYUN" = "y" ]; then
    ...
    if $DOCKER_CMD pull crpi-z60uur6y0xgl3fgs.cn-chengdu.personal.cr.aliyuncs.com/aura-grid/aura-grid-pro:latest; then
        echo -e "${BLUE}[*] 正在本地对齐官方容器标识...${NC}"
        $DOCKER_CMD tag crpi-z60uur6y0xgl3fgs.cn-chengdu.personal.cr.aliyuncs.com/aura-grid/aura-grid-pro:latest ghcr.io/24kbrother/aura-grid-pro:latest
...
if [ "$CONFIRM" != "yes" ]; then
    echo -e "\033[0;31m[ABORT] 用户取消升级。\033[0m"
    exit 0
fi
...
if [ -f "$COMPOSE_FILE" ]; then
    if grep -q "ghcr.io/24kbrother/aura-grid-pro" "$COMPOSE_FILE"; then
        sed -i.original -E 's|(image:[[:space:]]*ghcr\.io/24kbrother/aura-grid-pro):.*|\1:latest|g' "$COMPOSE_FILE" 2>/dev/null || true
    fi
fi
...
if [ "$UPDATE_SUCCESS" = "true" ]; then
    ...
    echo -e "\033[0;31m[ERROR] 升级失败，正在恢复身份保护层...\033[0m"
    [ -f "./device.id.bak" ] && mv ./device.id.bak ./data/device.id
    [ -f "./license.jwt.bak" ] && mv ./license.jwt.bak ./data/license.jwt
    exit 1
fi
...
echo -e "⚠️  ${RED}温馨提示: 请勿在该目录下重复执行一键脚本，谨防目录嵌套套娃！${NC}"
echo -e "${GREEN}==================================================${NC}"
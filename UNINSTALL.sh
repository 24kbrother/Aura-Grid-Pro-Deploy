...
# 硬核校验：确保没有任何残留容器（支持带后缀的容器，如 aura-grid-pro-24k）
echo -e "${GREEN}🔍 正在执行深度扫描以清理残留容器...${NC}"
# 获取所有以 aura- 开头的容器 ID
TARGET_CONTAINERS=$(docker ps -a --format '{{.Names}}' | grep -E "^aura-(grid|redis)-(pro|lite)")

if [ -n "$TARGET_CONTAINERS" ]; then
    for container in $TARGET_CONTAINERS; do
        echo -e "${YELLOW}⏳ 正在移除发现的容器: ${container}${NC}"
        docker rm -f "$container" >/dev/null 2>&1
        echo -e "${GREEN}✅ 容器 ${container} 已彻底移除。${NC}"
    done
fi
